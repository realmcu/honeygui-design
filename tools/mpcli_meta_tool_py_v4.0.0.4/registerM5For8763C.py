...
class RegisterM5For8763C(RegisterBase):
    ...
    def setup(self):
        ...
        if cmd_hdl.run_into_mp_loader() != ERR_OK:
            ...
            if self._config.get_ic_type() == "IC_8763C_V2":
                ...
                if (self._config.CONFIG_UPDATE_RAM_PATCH_ENABLE == True) or \
                        ((cmd_hdl.get_wire_mode() == SINGLE_WIRE_MODE) and \
                         self._cmdParser.is_flash_operation() == True):
                    mem_read_ret = cmd_hdl.read_disable_ram_patch_auth_val()
                    if mem_read_ret != ERR_OK:
                        LOG_ERR("Error: mem read efuse disable_ram_patch_auth value fail!")
                        return mem_read_ret

                    ram_patch_path = self.__get_fw_ram_path_path()
    ...
    def __hci_reboot(self):
        ...
        invalid_password = "00:11:22:33:44:55:66:77:88:99:AA:BB:CC:DD:EE:FF"

        return cmd_hdl.send_dbg_passwd(args.comport,
                                       cfg_info_dict[CFG_DICT_E.SERIAL_INFO][UART_ITEM_E.OPEN_BAUDRATE],
                                       invalid_password)
