...
class FlashFileParser8763D(object):
    def __init__(self, config):
        self._config = config
        self.FMC_MAIN0_ADDR = 0x02000000
        self.FMC_MAIN0_UNCACHEABLE_ADDR = 0x04000000
    ...
    def reset_flash_address(self,address):
        if (address > self.FMC_MAIN0_ADDR) and (address < self.FMC_MAIN0_UNCACHEABLE_ADDR):
            address = (address&(~self.FMC_MAIN0_ADDR))|(self.FMC_MAIN0_UNCACHEABLE_ADDR)
        return address
    ...
    def parseJson(self, cfg_path):
        ...
        if mptoolconfig_Node.__contains__("appimage"):
            ...
            LOG_ERR("Error: can not find appimage node in json file %s." % cfg_path)
            return cfg_info_dict
