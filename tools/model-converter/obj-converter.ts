...
export class OBJConverter {
    convert(objPath: string, binPath: string, txtPath: string, outputDir?: string): void {
        ...
        let allRectangle = true;
        let allTriangle = true;

        for (const numVerts of model.faceNumVerts) {
            if (numVerts !== 4) allRectangle = false;
            if (numVerts !== 3) allTriangle = false;
        }

        if (allRectangle) {
            faceType = FaceType.RECTANGLE;
        } else if (allTriangle) {
            faceType = FaceType.TRIANGLE;
        } else {
            faceType = FaceType.MIXED;
        }
        ...
        header.writeUInt8(ModelType.OBJ, 2);
