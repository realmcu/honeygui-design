/**
 * hg_slider event generator
 */
import { Component } from '../../../hml/types';
import { LvglEventCodeGenerator } from './LvglEventCodeGenerator';

export class LvglSliderEventGenerator implements LvglEventCodeGenerator {
  ...
  getEventCallbackImpl(component: Component): string[] {
    ...
    code += `        int32_t value = lv_slider_get_value(obj);\n`;
    code += `        LV_LOG_USER("Slider value: %d", (int)value);\n`;
    code += `    }\n`;
    code += `}\n`;
    return [code];
  }
