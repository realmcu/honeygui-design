...
export function useWebviewUri(relativePath: string | undefined): string | undefined {
  const [webviewUri, setWebviewUri] = useState<string | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (message.command === 'webviewUriConverted' && message.requestId === requestId) {
        if (!message.error && currentPathRef.current === relativePath) {
 ...
 * 清除 URI 缓存
 */
export function clearUriCache(path?: string): void {
  if (path) {
