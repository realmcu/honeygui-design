...
bool lv_wayland_window_decoration_create(struct window *window, struct graphic_object *decoration,
                                         int window_width,
                                         int window_height);
...
#endif
lv_result_t lv_wayland_xdg_shell_create_window(struct lv_wayland_context *app,
                                               struct window *window,
                                               const char *title);
...
void lv_wayland_xdg_shell_handle_pointer_event(struct lv_wayland_context *app, uint32_t serial,
                                               uint32_t button,
                                               uint32_t state);
...
void lv_wayland_shm_flush_partial_mode(lv_display_t *disp, const lv_area_t *area,
...
void lv_wayland_dmabuf_set_interface(dmabuf_ctx_t *context, struct wl_registry *registry,
                                     uint32_t name,
                                     const char *interface, uint32_t version);
...
void lv_wayland_dmabuf_on_graphical_object_destruction(dmabuf_ctx_t *context,
                                                       struct graphic_object  *obj);
...
lv_result_t lv_wayland_dmabuf_resize_window(dmabuf_ctx_t *context, struct window *window, int width,
                                            int height);
...
void lv_wayland_dmabuf_flush_full_mode(lv_display_t *disp, const lv_area_t *area,
                                       unsigned char *color_p);
...
void lv_wayland_cache_apply_areas(struct window *window, void *dest, void *src,
