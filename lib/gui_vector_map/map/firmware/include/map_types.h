...
extern "C" {
#endif
#define USE_HONEY_GUI
#ifdef USE_HONEY_GUI
#include "gui_api_os.h"
void *gui_lower_calloc(size_t num, size_t size);
#define MAP_MALLOC gui_lower_malloc
#define MAP_FREE gui_lower_free
#define MAP_CALLOC gui_lower_calloc
#define MAP_REALLOC gui_lower_realloc
#define MAP_PRINTF gui_log
#define MAP_FPRINTF MAP_PRINTF
#define sleep_ms(ms) (void *)ms;
#define MAP_TIME (time_t)0;
#else
 ...
 */
static inline bool map_get_edge_lane_config(const map_t *map, const map_edge_t *edge, lane_config_t *config)
{
    if (!map || !edge || !config)
    {
        return false;
