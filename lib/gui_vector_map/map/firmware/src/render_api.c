...
#ifndef USE_HONEY_GUI
#define STB_TRUETYPE_IMPLEMENTATION
#endif
#include "stb_truetype.h"

#ifdef USE_TRVG
/* Use TRVG (Triton Vector Graphics) software renderer */
#include "trvg/trvg.h"


#elif defined(USE_NANOVG)
/* Use real NanoVG with AGGE software renderer backend */
#include "nanovg.h"
#include "nanovg_agge.h"

#define nvgCreate(w, h, stride, fmt, data) nvgCreateAGGE(w, h, stride, fmt, data)
#define nvgDelete(ctx) nvgDeleteAGGE(ctx)

#else
/* Fallback: Simple software renderer without NanoVG */
...
static struct NVGcontext *nvgCreateFallback(int width, int height)
{
    struct NVGcontext *ctx = (struct NVGcontext *)MAP_MALLOC(sizeof(struct NVGcontext));
    if (!ctx)
    {
        return NULL;
    }
    ...
    ctx->width = width;
    ctx->height = height;
    ctx->pixels = (unsigned char *)MAP_CALLOC(width * height * 4, 1);
    if (!ctx->pixels)
    {
        MAP_FREE(ctx);
        return NULL;
...
static void nvgDeleteFallback(struct NVGcontext *ctx)
{
    if (ctx)
    {
        MAP_FREE(ctx->pixels);
...
#define nvgCreate(w, h, stride, fmt, data) nvgCreateFallback(w, h)
#define nvgDelete(ctx) nvgDeleteFallback(ctx)

#endif /* USE_TRVG / USE_NANOVG / Fallback */
 ...
 * ============================================================================
 */
static unsigned char *pixels_malloc = 0;
renderer_t *renderer_create(int width, int height)
{
    ...
    r->height = height;
    int pixbyte = 4; /* RGBA */
    trvg_pixel_format_t format = TRVG_PIXEL_RGBA8888;
    /* Allocate pixel buffer (RGBA format) */
#ifdef USE_HONEY_GUI
    format = TRVG_PIXEL_RGB565;
    pixbyte = 2; /* RGB565 format */
    if (!pixels_malloc)
    {

        pixels_malloc = (unsigned char *)MAP_MALLOC(width * height * pixbyte + sizeof(gui_rgb_data_head_t));
    }
    r->pixels = pixels_malloc;

    //  r->pixels = (void *)DSP_SHM_ADDR;
    memset(r->pixels, 0, width * height * pixbyte + sizeof(gui_rgb_data_head_t));
    gui_rgb_data_head_t *head = (gui_rgb_data_head_t *)r->pixels;
    head->w = width;
    head->h = height;
    head->type = RGB565; // RGB data type
    r->pixels = r->pixels + sizeof(gui_rgb_data_head_t);
...
#ifdef USE_TRVG
    /* Initialize TRVG context */

    if (!trvg_init(&r->trvg, r->pixels, width, height,
                   width * pixbyte, format))
    {
        MAP_FREE(r->pixels);
        MAP_FREE(r);
        return NULL;
    }
    MAP_PRINTF("TRVG renderer initialized: %dx%d\n", width, height);
#elif defined(USE_NANOVG)
    /* Create NanoVG context with AGGE backend */
    /* stride = width * 4 bytes per pixel (RGBA) */
    r->vg = nvgCreate(width, height, width * 4, NVG_TEXTURE_RGBA, r->pixels);
    if (!r->vg)
    {
        MAP_FREE(r->pixels);
        MAP_FREE(r);
...
#else
    /* Create fallback software renderer */
    r->vg = nvgCreate(width, height, 0, 0, NULL);
    if (!r->vg)
    {
        MAP_FREE(r->pixels);
        MAP_FREE(r);
...
void renderer_clear(renderer_t *renderer, uint32_t color)
{
    ...
    {
        renderer->pixels[i * 4 + 0] = r;
        renderer->pixels[i * 4 + 1] = g;
        renderer->pixels[i * 4 + 2] = b;
        renderer->pixels[i * 4 + 3] = a;
    }
#endif
}
...
map_error_t render_save_png(renderer_t *renderer, const char *filepath)
{
    ...
    if (stbi_write_png(filepath, renderer->width, renderer->height, 4, renderer->pixels, renderer->width * 4))
    {
        return MAP_OK;
    }
...
static unsigned char *s_font_buffer = NULL;       /* Font buffer (allocated when loaded from file) */
...
const uint8_t *ttf_font_file_data_address = (const uint8_t *)0x70dd1000;  /* Example address, replace with actual */
size_t ttf_font_file_data_size = 447404;  /* Example size, replace with actual */
void render_text(renderer_t *renderer, float x, float y, const char *text, float size, uint32_t color)
{
    if (!renderer || !text)
    {
        return;
    ...
    /* Use stb_truetype to render text directly to the pixel buffer */
    if (!s_font_loaded)
    {
#ifdef USE_HONEY_GUI
        /* MCU environment: font should be set via render_set_font_from_memory() before calling render_text */
        // MAP_PRINTF("WARNING: render_text called but font not loaded. Use render_set_font_from_memory() first.\n");
        // return;
        render_set_font_from_memory(ttf_font_file_data_address, ttf_font_file_data_size); //NotoSansSC-Medium_subset.ttf
#else
        /* Non-MCU environment: try to load font from file system */
        const char *font_paths[] =
        {

            "data/fonts/NotoSansSC-Medium_subset.ttf",
        ...
        if (fp)
        {
            ...
            fseek(fp, 0, SEEK_SET);
            s_font_buffer = (unsigned char *)MAP_MALLOC(length);
            if (s_font_buffer)
            {
                fread(s_font_buffer, 1, length, fp);
                fclose(fp);

                s_font_data = s_font_buffer;
                s_font_from_memory = 0;

                if (stbtt_InitFont(&s_font, s_font_data, 0))
                {
                    s_font_loaded = 1;
                    MAP_PRINTF("Font initialized successfully!\n");
                }
                else
                {
                    MAP_PRINTF("ERROR: Failed to initialize font!\n");
                    MAP_FREE(s_font_buffer);
                    s_font_buffer = NULL;
                    s_font_data = NULL;
                }
            ...
            {
                fclose(fp);
                MAP_PRINTF("ERROR: Failed to allocate memory for font!\n");
            }
        }
        else
        {
            MAP_PRINTF("ERROR: Could not load any font file!\n");
        }
#endif /* USE_HONEY_GUI */
    }
    ...
    while (*text)
    {
        ...
        // Handle basic UTF-8 for Chinese characters (simple version)
        if ((code & 0x80) != 0)
        {
        ...
        {
#ifdef USE_HONEY_GUI
            /* RGB565 format for MCU environment */
            uint16_t *pixels16 = (uint16_t *)renderer->pixels;
            /* Pre-convert foreground color to RGB565 */
            ...
            for (int j = 0; j < out_h; j++)
            {
                for (int i = 0; i < out_w; i++)
                {
                    ...
                    if (px >= 0 && px < renderer->width && py >= 0 && py < renderer->height)
                    {
                        ...
                        {
                            /* Alpha blending with RGB565 background */
...
#else
            /* RGBA format for non-MCU environment */
            for (int j = 0; j < out_h; j++)
            {
                for (int i = 0; i < out_w; i++)
                {
...
#define RENDER_AREA(area_idx) do { \
    ...
    if (use_area_spatial_index)
    {
        ...
        MAP_PRINTF(">>> AREA SPATIAL INDEX ACTIVE: checked %u/%u areas (%.2f%%), rendered %u, cells %u/%u <<<\n",
                   ...
                   map->header.grid_cols * map->header.grid_rows);
    }
    else
    {
