...
const uint8_t *trmap_file_data_address = (const uint8_t *)0x704d1000;  /* Example address, replace with actual */
size_t trmap_file_data_size = 8311352;  
map_t *map_load(const char *filepath)
{
    (void)filepath;
#if ! defined(USE_HONEY_GUI)
    ...
    fclose(fp);
#else
    MAP_FPRINTF("Loading map from memory-mapped address\n");
    const uint8_t *data = trmap_file_data_address;
    size_t file_size = trmap_file_data_size;

#endif
    /* Load from memory */
    map_t *map = map_load_from_memory(data, file_size, true);  /* owns_data = true */
...
map_t *map_load_from_memory(const uint8_t *data, uint32_t size, bool owns_data)
{
    ...
    if (version == 1)
    {
        ...
        for (uint32_t i = 0; i < edge_count; i++)
        {
            map->edges[i].from_node = *(uint32_t *)(ptr + i * 12);
            map->edges[i].to_node = *(uint32_t *)(ptr + i * 12 + 4);
            map->edges[i].distance = *(float *)(ptr + i * 12 + 8);
                                ...
                                transport_mode_t mode)
{
    ...
    float *g_score_real = (float *)MAP_MALLOC(node_count * sizeof(float)); /* Real distance for output */
