...
const CreateProjectPanel_1 = require("../../src/designer/CreateProjectPanel");
// 模拟fs模块
jest.mock('fs', () => ({
    existsSync: jest.fn(),
    mkdirSync: jest.fn(),
...
describe('CreateProjectPanel', () => {
    ...
    describe('createOrShow', () => {
        it('should create a new panel if no panel exists', () => {
            // 确保没有当前面板
            CreateProjectPanel_1.CreateProjectPanel.currentPanel = undefined;
            // 调用方法
            CreateProjectPanel_1.CreateProjectPanel.createOrShow(mockContext);
            // 验证面板创建
            expect(vscode.window.createWebviewPanel).toHaveBeenCalledTimes(1);
            expect(vscode.window.createWebviewPanel).toHaveBeenCalledWith('createProject', 'Create New Project', vscode.ViewColumn.One, expect.any(Object));
            // 验证当前面板已设置
            expect(CreateProjectPanel_1.CreateProjectPanel.currentPanel).toBeDefined();
        });
        it('should reveal existing panel if panel already exists', () => {
            ...
            // 设置当前面板
            CreateProjectPanel_1.CreateProjectPanel.currentPanel = existingPanel;
            // 调用方法
            CreateProjectPanel_1.CreateProjectPanel.createOrShow(mockContext);
            // 验证没有创建新面板
    ...
    describe('_selectProjectFolder', () => {
        it('should handle folder selection successfully', async () => {
            // 创建面板实例
            const panel = CreateProjectPanel_1.CreateProjectPanel.createOrShow(mockContext);
            // 调用私有方法（通过访问私有属性）
            await panel._selectProjectFolder();
            // 验证showOpenDialog调用
    ...
    describe('_createProject', () => {
        ...
        beforeEach(() => {
            // 创建面板实例
            panel = CreateProjectPanel_1.CreateProjectPanel.createOrShow(mockContext);
        ...
        it('should create project with valid config', async () => {
            ...
            // 验证信息消息
            expect(vscode.window.showInformationMessage).toHaveBeenCalledWith('Creating project: TestProject...');
            // 验证全局状态更新
            expect(mockGlobalState.update).toHaveBeenCalledWith('pendingProjectActivation', expect.objectContaining({
                projectPath: '/mock/save/location/TestProject',
                projectName: 'TestProject'
            }));
            // 验证打开文件夹
            expect(vscode.commands.executeCommand).toHaveBeenCalledWith('vscode.openFolder', expect.any(Object), false);
        });
    ...
    describe('_createProjectStructure', () => {
        ...
        it('should create project directories and files', async () => {
            ...
            // 调用私有方法
            await panel._createProjectStructure(projectPath, projectName, appId, resolution, minSdk, pixelMode);
            // 验证目录创建
    ...
    describe('dispose', () => {
        it('should clean up resources properly', () => {
            ...
            panel._disposables = [mockDisposable];
            // 调用dispose
            panel.dispose();
            // 验证当前面板已清除
            expect(CreateProjectPanel_1.CreateProjectPanel.currentPanel).toBeUndefined();
            // 验证面板已释放
...
//# sourceMappingURL=CreateProjectPanel.test.js.map