...
本文档记录 HoneyGUI Design 设计器导出到嵌入式平台的三种方案。

## 方案概览

| 方案 | 导出格式 | 固件改动 | 资源占用 | 适用场景 |
|------|---------|---------|---------|---------|
| 方案一 | C 代码 | 每次都要 | 最小 | 资源受限、UI 固定 |
| 方案二 | HML (XML) | 不需要 | 中等（~10KB 解析器） | 需要 OTA 更新 UI |
| 方案三 | HML (BIN) | 不需要 | 小（~2KB 解析器） | 需要 OTA + 资源受限 |
...
| 解析器 | ROM | 特点 |
|--------|-----|------|
| ezxml | ~10KB | DOM 模式，API 简洁 |
| yxml | ~3KB | SAX 流式，零动态内存 |
| mxml | ~30KB | 功能完整 |
...
1. [ ] 在 project.json 中添加 deployment 配置
2. [ ] 实现"导出嵌入式部署包"命令
3. [ ] 支持方案一：C 代码导出（已有）
4. [ ] 支持方案二：HML 导出
5. [ ] 支持方案三：BIN 编译导出
6. [ ] 提供嵌入式侧的 HML 加载器参考实现
